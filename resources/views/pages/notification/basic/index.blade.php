...
@section("headerInformation")
    <h4 class="nk-block-title">
        @lang("messages.notification-basic")
    </h4>
    <div class="nk-block-des">
        <p>
            @lang("messages.notification-basic-description")
        </p>
    </div>
@endsection
    ...
    <div class="nk-notification">
        @if(count($notifications) > 0)
            @foreach($notifications as $notification)
                @if($notification->data['type'] == "audit")
                       ...
                       class="nk-notification-item dropdown-inner {{!is_null($notification->read_at) ? 'been-read' : 'unread'}}">
                        ...
                        <div class="nk-notification-content">
                            <div class="nk-notification-text">
                                @lang("messages.type") : @lang("messages.audit")
                                @lang("messages.from") {{ $notification->data['data']['user_name'] }}
                                <br>
                                @lang("messages.on-table") : @lang("messages".$notification->data['data']['table_name'])
                                <br>
                                @lang("messages".$notification->data['data']['event'])
                            </div>
                ...
                @elseif($notification->data['type'] == "print")
                    <a href="{{url()->to($notification->data['data']['route_name'])}}"
                       class="nk-notification-item dropdown-inner {{!is_null($notification->read_at) ? 'been-read' : 'unread'}}">
                        <div class="nk-notification-icon">
                            <em class="icon icon-circle bg-warning-dim ni ni-curve-down-right"></em>
...
{{--                <div class="col-12">
                    <div class="form-group">
                        <label class="form-label" for="type-filter">
                            @lang("messages.username-notification")
                        </label>
                        <div class="form-control-wrap">
                            <select id="type-filter" name="filter[type]"
                                    data-placeholder="{{__('messages.chose_option')}}"
                                    data-search="on" class="form-select js-select2">
                                <option value="">
                                    {{__('messages.chose_option')}}
                                </option>
                            </select>
                        </div>
                    </div>
